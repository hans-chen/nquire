...
local function open(display)
	...
	local zoom = config:get("/dev/display/zoom")
	local reverse = config:get("/dev/display/reverse") == "true"
	local fullscreen = config:get("/dev/display/fullscreen") == "true"
	local showcursor = config:get("/dev/display/show_cursor") == "true"

	require("dpydrv")
	
	display.drv = dpydrv.new()
	local w, h, c = mode:match("(%d+)x(%d+)(.)")
	local update_freq, err = display.drv:open(w, h, c == 'c', zoom, fullscreen, reverse, showcursor)
...
function new()
	...
	config:add_watch("/dev/display", "set", 
