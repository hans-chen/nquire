...
local function load_db(config)
	...
	local fd, err = io.open(config.fname_db, "r")
	if not fd then
...
local function save_db(config)
	...
	if not fd then
		logf(LG_WRN, "Could not open config db %s: %s", config.fname_db, err)
		return
	end
	...
	os.rename(fname_tmp, config.fname_db)
...
local function set(config, fid, value)
	local node = config:lookup(fid)
	if node then
