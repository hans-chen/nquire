...
local function handle_barcode_normal(barcode)
...
local function handle_barcode_programming(barcode)
	...
	if tmp then 
		logf(LG_INF, "Scanned 'beeper volume' barcode, set volume to %s", tmp)
	...
	if tmp then 
		logf(LG_INF, "Scanned 'beeper sound type' barcode, set sound type to %s", tmp)
	...
	if tmp then 
		logf(LG_INF, "Scanned 'display contrast' barcode, set contrast to %s", tmp)
...
local function on_barcode(event, cit)
	...
	elseif barcode_mode == "programming" then
		if barcode == "%*^$#%" then
			...
			handle_barcode_programming(barcode)
		end
	end
...
local function start(cit)
	...
	evq:register("cit_idle_msg", function(event, cit) cit:draw_idle_msg() end, cit)
	evq:register("cit_error_msg", function(event, cit) cit:draw_error_msg() end, cit)
